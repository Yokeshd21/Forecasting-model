...
def get_ai_insights(df, latest_reading, model_id="llama-3.3-70b-versatile"):
    ...
    try:
        ...
        return f"Error during AI analysis: {e}", None
...
with st.sidebar:
    ...
    st.selectbox(
        label   = "Navigation",
        options = ["Navigation", "Home", "Dashboard", "Reports"],
    ...
    service_choice = "Select the Service"
    model_id       = "llama-3.3-70b-versatile"   # safe default
    model_name     = "Llama 3.3 70B"             # safe default
    ...
    st.markdown("""
        ...
        <div style="display:flex;justify-content:space-between;
            ...
            <img src="https://i.im.ge/2026/03/17/eKAOPG.oie-png.png"
                 style="width:45%;height:auto;background:white;
...
main_func = "Select"
if func_choice == "Forecasting" and service_choice == "Google Cloud Service":
            ...
            if gen_ai:
                with st.spinner(f"Groq ({model_name}) is analysing your data..."):
                    ...
                    st.markdown(f"""
                        ...
                        {insights.split('OPTIMAL_VALUES:')[0]}
                    """, unsafe_allow_html=True)
                        ...
                        if not test_clean.empty:
                            with st.spinner("Calculating PMV predictions..."):
                                ...
                                for idx, row in test_clean.iterrows():
                                    ...
                                    # Formula actual
                                    actual_pmv = hm.estimate_pmv_from_sensors(row_dict)
                                    actual_pmv_list.append(actual_pmv)
                                    
                                    # Model forecast
                                    new_input = np.array([row_dict[f] for f in HVAC_FEATURES], dtype='float32')
                                    forecast_pmv = hm.predict_lstm(model, None, feat_scaler, pmv_scaler, current_history, new_input)
                                    forecast_pmv_list.append(forecast_pmv)
                                    
                                    current_history = np.vstack([current_history[1:], new_input])
                                
                                test_clean['Actual PMV (Formula)'] = actual_pmv_list
                                ...
                                """)
                                st.dataframe(test_clean[['Actual PMV (Formula)', 'AI Forecast PMV (LSTM)']].style.format("{:.2f}").background_gradient(cmap="coolwarm"), use_container_width=True)
                                
                                fig, ax = plt.subplots(figsize=(10, 4))
                                ax.plot(range(len(test_clean)), test_clean['Actual PMV (Formula)'].values, label='Actual PMV (Formula)', color='blue')
                                ax.plot(range(len(test_clean)), test_clean['AI Forecast PMV (LSTM)'].values, label='AI Forecast PMV', color='darkorange', linestyle='--')
                                ax.set_title("Actual vs Forecasted PMV")
